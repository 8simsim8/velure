    ...
    function blockInstagram() {
        ...
        var feed = new Instafeed({
            ...
            filter: function(image) {
                ...
                var tempArray = separateCaption(image);

                image.caption_without_tags = tempArray[0];
                image.caption_with_tags = tempArray[1];
            ...
            ' lg-3"><p class="insta-header"><span class="insta-logo">VelurSpaSumy</span><span' +
            ' class="insta-date-create">{{model.data_create}}</span></p><img' +
            ' src="{{image}}"/><div class="insta-caption"><p' +
            ' class="insta-caption__text">{{model.caption_without_tags}}</p><p' +
            ' class="insta-caption__tags">{{model.caption_with_tags}}</p></div></div>',
        });
        ...
        function separateCaption(image) {
            var newCaptionWithTag = [];
            var newCaptionWithoutTag = [];
            ...
            if (image.caption && image.caption.text && image.caption.text.length > 0) {
                ...
                for (var i=0; i < captionParts.length ; i++) {
                    if (captionParts[i].charAt(0) !== '#') {
                        newCaptionWithoutTag.push(captionParts[i]);
                    } else {
                        newCaptionWithTag.push(captionParts[i]);
                    }
                }
            }

            resArray[0] = newCaptionWithoutTag.join(' ');
            resArray[1] = newCaptionWithTag.join(' ');
